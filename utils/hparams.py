...
def override_config(old_config: dict, new_config: dict):
    for k, v in new_config.items():
        if isinstance(v, dict) and k in old_config:
            if old_config[k] is None:
                # If old_config[k] is None, replace it with an empty dict
                old_config[k] = {}
            if isinstance(old_config[k], dict):
                override_config(old_config[k], new_config[k])
            else:
                # If old_config[k] is not a dict but v is a dict, replace it
                old_config[k] = v
        else:
            old_config[k] = v
